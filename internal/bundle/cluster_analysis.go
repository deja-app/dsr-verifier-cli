...
type TemporalClusteringResult struct {
	...
	// WindowStart and WindowEnd are POINTERS so that omitempty actually omits
	// them. As time.Time values they were never omitted: encoding/json's
	// omitempty has no effect on struct types, so an unset window serialised as
	// "0001-01-01T00:00:00Z" — a specific datetime that never happened, written
	// into an audit artefact an auditor archives.
	//
	// They are assigned only when a cluster is detected, which means the
	// fabricated timestamp appeared on the COMMON path: the bestCount < 3 early
	// return, and every bundle whose anomalies did not clear the threshold.
	// ClusterAnalysis is always non-nil, so that was essentially every bundle
	// verification report ever produced.
	//
	// nil means "no burst window was identified". It does not mean midnight on
	// 1 January of year 1.
...
func testTemporalClustering(anomalies []Anomaly) TemporalClusteringResult {
	...
	if pLT001 {
		...
		windowFinish := bestStart.Add(windowEnd)
		res.WindowStart = &windowStart
		res.WindowEnd = &windowFinish
		res.PValueLT = "<0.001"
	}
	return res
}
